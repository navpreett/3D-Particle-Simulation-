    ...
    pub fn update(&mut self, ts: f32) {
        {
            ...
            (1..hash_table.len()).into_par_iter().for_each(|i| {
            ...
            self.current_particles.par_iter().enumerate().for_each(|(i, sphere)| {
                let index = hash(cell_coord(sphere.position)) % hash_table_length;
            ...
            self.current_particles.par_extend(self.previous_particles.par_iter().map(|&mut particle| {
                let mut total_force = vec3(0.0, 0.0, 0.0);
                for x_offset in -1..=1 {
                    for y_offset in -1..=1 {
                        for z_offset in -1..=1 {
                            let offset = vec3(x_offset as _, y_offset as _, z_offset as _) * self.world_size;
                                ...
                                for y_cell_offset in -1..=1 {
                                    for z_cell_offset in -1..=1 {
                                            ...
                                            ..hash_table[index + 1].load(Relaxed)]
                                        {
                                            ...
                                            let sqr_distance = relative_position.length_squared();
                                                ...
                                                && sqr_distance < self.particle_effect_radius * self.particle_effect_radius
                                            {
                                                ...
                                                let f = force(
                                                    distance,
                                                    self.attraction_matrix[particle.id * self.id_count + other_particle.id],
                                                );
        ...
        self.current_particles.par_iter_mut().for_each(|particle| {
            ...
            let wrap_position = |pos: &mut f32| {
                if *pos > self.world_size * 0.5 {
                    *pos -= self.world_size;
                } else if *pos < -self.world_size * 0.5 {
                    *pos += self.world_size;
                }
            };

            wrap_position(&mut particle.position.x);
            wrap_position(&mut particle.position.y);
            wrap_position(&mut particle.position.z);
        });
    }
}
